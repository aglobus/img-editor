...
# 1D FT
#
# You may use this code in  own forwardFT() and backwardFT() functions.
...
def forwardFT( image ):
  '''
  F(u,v) is separable:
  1. Compute F(u,y) for all (u,y)
  2. Use F(u,y) to compute F(u,v)
  '''

  # Apply the ft1D along one axis and use that result to compute along the other axis
  return np.apply_along_axis(ft1D, 0, np.apply_along_axis(ft1D, 1, image));
...
def inverseFT( image ):
  ...
  def ift1D(signal):
    return np.conj(ft1D(np.conj( signal )) / signal.size)

  # Same idea as forwardFT, except we apply ift1D where ift1D computes the inverse FT from the forward transform by taking the conjugate of the signal, applying that to the forward transform, dividing by the signal's size and taking the conjugate of that

  return np.apply_along_axis(ift1D, 0, np.apply_along_axis(ift1D, 1, image.copy()));
...
def multiplyFTs( image, filter ):
  ...
  # Shift the entire filter down by M/2 and to the left by N/2 first before multiplying
  # Equivalent to walking through entire array and multiplying by -1^u*-1^v
...
def modulatePixels( image, x, y, isFT ):
  ...
  # Returns gaussian given a distance and some radius
  def G(dst, R): # dst = distance from center, R = Radius
    return (math.exp(-0.5 * (dst**2)/((R/2)**2)))

  # For all points within radius of the clicked (x,y)
  for i in range(-radius, radius):
    for j in range(-radius, radius):
      dst = math.sqrt(i**2 + j**2)
      if dst <= radius: # point is within circle
        gaus = G(dst, radius)
        igaus = 1 - gaus # subtractive
        mgaus = 1 + (0.1)*gaus # additive

        if isFT:
          # Calc magnitude & phase
          v = image[y+j][x+i]
          A = math.sqrt(v.real**2 + v.imag**2)
          phase = math.atan(-v.imag/v.real)

          if editMode == 's':
            A = math.exp(math.log(A + 1) * igaus) - 1
          if editMode == 'a':
            A = math.exp(math.log(A + 1) * mgaus) - 1

          image[y+j][x+i] = complex(A*math.cos(phase), A*math.sin(phase))
          image[-y+j][-x+i] = image[y+j][x+i] # apply to the opposite side
        else: # not a FT, apply regular gaussian
          if editMode == 's':
            image[y+j][x+i] = image[y+j][x+i] * (igaus)
          if editMode == 'a':
            image[y+j][x+i] = image[y+j][x+i] * (mgaus)
